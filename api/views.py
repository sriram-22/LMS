...
from .models import (
    User, Course, Enrollment, CourseVideo, CourseComment,
    CourseLike, CourseRating, CourseProgressTracking, Quiz,
    Question, QuizAttempt
    )
from .serializer import (
    Loginserializer, UserSerializer,
    UpadateUserPasswordSerializer, CourseSerializer,
    EnrollmentSerializer, CourseVideoSerializer,
    CourseCommentSerializer, CourseLikeSerializer,
    CourseRatingSerializer,
    CourseProgressTrackingSerializer, QuestionSerializer,
    QuizSerializer, QuizAttemptSerializer
    )
from rest_framework_simplejwt.tokens import RefreshToken
from .permissions import (
    IsAdminUserRole, IsInstructorUserRole,
    IsStudentUserRole, IsAdminOrInstructor, IsUser,
    IsUserorAdmin,
    IsAdminOrInstructorOrStudentRelatedToCourse,
    IsAdminOrInstructorRelatedToCourse,
    IsStudentRelatedToCourse, IsInstructorRelatedToCourse
    )
...
class BaseAPIView(APIView):
    ...
    def handle_exception(self, exc):
        ...
        if isinstance(exc, PermissionDenied):
            return Response({"error h_exception": "You do not have permission to perform this action"},
                status=status.HTTP_403_FORBIDDEN,)
        
        if isinstance(exc, Http404):
            return Response({f"error h_exception: {str(exc)}"},
...
class UserAPIView(BaseAPIView):
    ...
    def patch(self, request, pk):
        ...
        serializer = UserSerializer(user, data=request.data, fields = ["id", "username", "email", "password","password2", "role"], partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class UpdateUserPasswordAPIView(APIView):
    def put(self, request):
        serializer = UpadateUserPasswordSerializer(data = request.data, context={'request':request})
        if serializer.is_valid():
            serializer.save()
...
class CourseAPIView(BaseAPIView):
    ...
    def get(self, request, pk=None):
        if pk:
            ...
            if q:
                ...
                course = Course.objects.annotate(rank=SearchRank(search_vector, search_query)).filter(rank__gte=0.01).order_by('-rank')
            else:
                course = Course.objects.all()

            serializer = CourseSerializer(course, fields = ["id", "name", "description"], many=True)
            return Response(serializer.data)
    
    def post(self, request):
        serializer = CourseSerializer(data=request.data, fields = ["name", "description","instructors"])
        if serializer.is_valid():
            serializer.save()
    ...
    def put(self, request, pk):
        ...
        serializer = CourseSerializer(course, data=request.data, fields = ['name', 'description', "instructors"])
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class CourseInstructorsAPIView(APIView):
    ...
    def get(self, request, pk):
        try:
            course = Course.objects.get(id=pk)
            serializer = CourseSerializer(course, fields = ["id","name","description","instructors"])
            return Response(serializer.data)
        except Course.DoesNotExist:
            return Response({"error": "Course not found"}, status=status.HTTP_404_NOT_FOUND)
...
class StudentEnrollmentAPIView(APIView):
        ...
        if pk:
            try:
                ...
                if enrollment.student.id != student.id:
                    return Response({"error": "You are not authorized to view this enrollment"}, status=status.HTTP_403_FORBIDDEN)
    ...
    def delete(self, request, pk):
        ...
        if enrollment.student.id != request.user.id:
            return Response({"error": "You are not authorized to delete this enrollment"}, status=status.HTTP_403_FORBIDDEN)
        enrollment.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
...
class EnrollmentAPIView(APIView):
    ...
    def patch(self, request, pk):
        try:
            ...
            return Response({"message": "Enrollment not found"}, status=status.HTTP_404_NOT_FOUND)
        serializer = EnrollmentSerializer(enrollment, data = request.data, fields = ["instructor", "status"], partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def get(self, request, pk):
        if pk:
            ...
            if not course_videos:
                return Response({"error": "CourseVideos not found"}, status=status.HTTP_404_NOT_FOUND)
...
class CourseCommentAPIView(APIView):
    ...
    def post(self, request, pk):
        ...
        course = self.get_object(pk)
        data["course"] = course.id
        serializer = CourseCommentSerializer(data=data, context={"request": request})  # Pass context
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def patch(self, request, pk):
        course = self.get_object(pk)
        try:
            comment = CourseComment.objects.get(id=request.data.get("id"), course=course, user=request.user)
    ...
    def delete(self, request, pk):
        ...
        try:
            comment = CourseComment.objects.get(id=comments_id, course=course, user=request.user)
        except CourseComment.DoesNotExist:
            return Response({"error": "Comment not found"}, status=status.HTTP_404_NOT_FOUND)
        comment.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
...
class StudentCourseProgressTrackingAPIView(APIView):
    ...
    def get(self, request, pk):
        course = self.get_object(pk)
        try:
            course_progress = CourseProgressTracking.objects.get(course=course, student=request.user)
        except CourseProgressTracking.DoesNotExist:
            return Response({"error": "CourseProgressTracking not found"}, status=status.HTTP_404_NOT_FOUND)
    ...
    def put(self, request, pk):
        ...
        if not completed_videos_ids:
            return Response({"error": "completed_videos field is required"}, status=status.HTTP_400_BAD_REQUEST)
        
        completed_videos = CourseVideo.objects.filter(id__in=completed_videos_ids, course=course)
        ...
        serializer = CourseProgressTrackingSerializer(course_progress, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def patch(self, request, pk):
        ...
        serializer = CourseProgressTrackingSerializer(course_progress, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class InstructorStudentsCourseProgressTrackingAPIView(APIView):
    ...
    def get(self, request, pk):
        course = self.get_object(pk)
        try:
            instructor_students = Enrollment.objects.filter(course=course, instructor=request.user, status="approved")
        except Enrollment.DoesNotExist:
            return Response({"error": "user not found"}, status=status.HTTP_404_NOT_FOUND)
        
        students = [student.student for student in instructor_students]
        students_progress = CourseProgressTracking.objects.filter(course=course, student__in=students)
...
class QuizAPIView(APIView):
    ...
    def get(self, request, pk=None):
        ...
        serializer = QuizSerializer(quizzes, fields=["id","video","title","description"], many=True)
        return Response(serializer.data)
    ...
    def post(self, request):
        ...
        serializer = QuizSerializer(fields = ["video", "title", "description"], data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def put(self, request, pk):
        quiz = self.get_object(pk)
        serializer = QuizSerializer(quiz, fields = ["video", "title", "description"], data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class QuizQuestionAPIView(APIView):
    ...
    def post(self, request):
        ...
        serializer = QuestionSerializer(fields = ["quiz", "question", "marks"],data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def put(self, request, pk):
        try:
            ...
            return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
        serializer = QuestionSerializer(question, fields = ["question", "marks"], data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class QuizAttemptAPIView(APIView):
    ...
    def get_object(self, pk):
        ...
        if not (Enrollment.objects.filter(course=quiz_attempt.quiz.video.course, student=student, status="approved", instructor= self.request.user.id).exists()):
            raise APIException({"error": "You are not authorized for this quiz attempt"})
        return quiz_attempt
    ...
    def post(self, request):
        ...
        quiz_serializer = QuizAttemptSerializer(fields = ["quiz", "student"], data=request.data)
        ...
        try:
            ...
            return Response({"message": "Your Quiz Attempt Saved Successfully"}, status=status.HTTP_201_CREATED)        
        except Exception as e:
            return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def put(self, request, pk):
        ...
        quiz_attempt_serializer = QuizAttemptSerializer(quiz_attempt, data=request.data, fields = ["answers","qualified_status"], partial=True)
